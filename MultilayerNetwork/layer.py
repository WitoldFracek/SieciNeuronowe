...
class Layer:
    def __init__(self, size: tuple[int, int], activation, act_derivative, init_weights_range=None, learning_rate=0.1):
        ...
        self.__bias = random.random() + 0.001
    ...
    def forward(self, x: np.ndarray) -> np.ndaray:
    ...
    def backward(self, next_dw, next_dz):
        dz = next_dw.T.dot(next_dz) * self.__act_derivative(self.__z_cache)
        dw = dz.dot(self.__a_cache.T) / self.__cluster_size
    ...
    def last_layer_operations(self, y):
        self.__dz_cache = self.__a_cache - y
